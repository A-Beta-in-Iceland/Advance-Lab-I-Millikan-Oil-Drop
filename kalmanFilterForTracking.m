...
%
function data = kalmanFilterForTracking(imname)
...
function trackSingleObject(param)
  ...
  while hasFrame(utilities.videoReader)
    ...
    if ~isempty(trackedLocation)
        time = [time, t];
        %we are only interested in the y axis
        pos = [pos, trackedLocation(2)];
    end
...
% param.segmentationThreshold = 0.00005; % smaller value resulting in noisy detections
...
function utilities = createUtilities(param)
  ...
  utilities.videoReader = VideoReader(imname);
